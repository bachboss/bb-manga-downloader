 ...
 * @author Bach
 */
public class Cococomic implements IBusOnePage {  // Done

    private static Pattern PATTERN_SERVER = Pattern.compile("\\?.*s=(\\d+)");
    private static Pattern PATTERN_PAGE = Pattern.compile("ServerList\\[(\\d+)\\]=\"(.+)\";");
    //
    private static final String BASED_URL = "http://www.cococomic.com";
    private static final String URL_JS_FILE = "http://www.cococomic.com/v3/i3.js";
    ...
    protected Document getDocument(String url) throws IOException {
        return HttpDownloadManager.getDocument(url, "gb2312");
    }
        ...
        for (Element e : xmlNodes) {
            if (e.html().contains("var PicListUrl")) {
                ...
                for (int i = 0; i < arrImgLink.length; i++) {
                    lstImage.add(new Image(i, baseImgUrl + arrImgLink[i], chapter));
                }
                break;
            }
        }
    ...
    private static String getImageBaseUrl(String chapterUrl) throws IOException {
        ...
        int server = 1;
        {
            Matcher m = PATTERN_SERVER.matcher(chapterUrl);
            if (m.find()) {
        ...
        }
        {
            ...
            do {
                Matcher m = PATTERN_PAGE.matcher(line);
                if (m.find()) {
